...
function pageads_admin() 
{
	?>
	<div class="wrap">
	<div id="icon-tools" class="icon32"></div>
	<h2>Publicidad Paginas - Configuraci&oacute;n</h2>
	<div class="clear"></div>
	<div id="Main-Ads">
	<?php
	global $wpdb;
	$wpdb->flush();
	$query=$wpdb->prepare("SELECT p.ads_id AS id, p.ads_name AS name, p.ads_maxwidth AS w, p.ads_maxheight AS h, b.anb_image as ads, b.anb_url as url, p.ads_resize as resize, b.anb_anbs_id AS size FROM wp_an_ads_publicidad AS p LEFT JOIN wp_an_ads_banners AS b ON p.ads_anb_id=anb_id LEFT JOIN wp_an_ads_banners_size AS s ON b.anb_anbs_id=s.anbs_id WHERE p.ads_anp_id=%d",3);
	...
	if($wpdb->num_rows>0) 
	{
		...
		{
			?>
			<form class="Cnt-Ads" id="Ads-<?=bin2hex($ad->id)?>" onsubmit="UpdatePublicidad('UpdPageContent', <?=bin2hex($ad->id)?>);return false;">
				<label>
					...
					<?php
						if($ad->resize==1) 
						{
							foreach($sizes as $size)
							{
								...
								{
									?>
						<option value="<?=bin2hex($resize->id)?>" <?=(!is_null($ad->size))?($ad->size==$size->id)?"selected":"":"";?> ><?=$size->name?></option>
									<?php
								}
							}
						}else 
...
add_action('wp_ajax_UpdPageContent', 'update_pubP');
add_action('wp_ajax_nopriv_UpdPageContent', 'update_pubP');

function update_pubP() 
{
	global $wpdb;
	if(($_POST['img']!="") && (intval($_POST['img'])!=0)) 
	{
		$id=intval(pack("H*",$_POST['id']));
		$query=$wpdb->prepare("SELECT * FROM wp_an_ads_publicidad WHERE ads_id=%d AND ads_anp_id=%d",$id,3);

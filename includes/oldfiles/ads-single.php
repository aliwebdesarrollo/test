...
function postads_admin() 
{
	?>
	<div class="wrap">
	<div id="icon-tools" class="icon32"></div>
	<h2>Publicidad Entradas - Configuraci&oacute;n</h2>
	<div class="clear"></div>
	<div id="Main-Ads">
	<?php
	global $wpdb;
	$wpdb->flush();
	$query=$wpdb->prepare("SELECT p.ads_id AS id, p.ads_name AS name, p.ads_maxwidth AS w, p.ads_maxheight AS h, b.anb_image as ads, b.anb_url as url, p.ads_resize as resize, b.anb_anbs_id AS size FROM wp_an_ads_publicidad AS p LEFT JOIN wp_an_ads_banners AS b ON p.ads_anb_id=anb_id LEFT JOIN wp_an_ads_banners_size AS s ON b.anb_anbs_id=s.anbs_id WHERE p.ads_anp_id=%d",4);
	...
	if($wpdb->num_rows>0) 
	{
		...
		{
			?>
			<form class="Cnt-Ads" id="Ads-<?=bin2hex($ad->id)?>" onsubmit="UpdatePublicidad('UpdSingleContent', <?=bin2hex($ad->id)?>);return false;">
				<label>
...
add_action('wp_ajax_UpdSingleContent', 'update_pubS');
add_action('wp_ajax_nopriv_UpdSingleContent', 'update_pubS');
...
function update_pubS() 
{
	...
	if(($_POST['img']!="") && (intval($_POST['img'])!=0)) 
	{
		...
		$query=$wpdb->prepare("SELECT * FROM wp_an_ads_publicidad WHERE ads_id=%d AND ads_anp_id=%d",$id,4);

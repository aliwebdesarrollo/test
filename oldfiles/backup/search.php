...
<?php if ( have_posts() ) : ?>
      <h3 class="pagetitle">Resultados de búsqueda de "<?php echo get_search_query(); ?>"</h3>
        <?php
         get_template_part( 'loop', 'category' );
        ?>
<?php else : ?>
        <div id="post-0" class="post no-results not-found">
          <h2 class="entry-title"><?php _e( 'No se encontraron resultados', 'twentyten' ); ?></h2>
          <div class="entry-content">
            <p><?php _e( 'No se encontraron coincidencias, por favor intente con otros criterios de búsqueda.', 'twentyten' ); ?></p>
            <?php get_search_form(); ?>
          </div><!-- .entry-content -->
        </div><!-- #post-0 -->
<?php endif; ?>
        ...
        <div class="boxes">
          <ul>
            ...
            <li>
              ...
              <p><?php if(has_post_thumbnail()) { ?><a href="<?php the_permalink() ?>" title="<?php the_title(); ?>"><?php the_post_thumbnail('home-thumb') ?></a><?php } ?>
              <?php $excerpt = get_the_excerpt(); echo string_limit_words($excerpt,16); ?></p>
              <div class="meta"><?php the_time('jS F Y') ?> | <a href="<?php the_permalink() ?>">Leer más...</a></div>
            </li>
            <?php endwhile; ?>
            <?php wp_reset_query(); ?>
          </ul>

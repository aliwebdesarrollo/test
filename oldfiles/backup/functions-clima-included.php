...
function an_clima_updlocinfo($loc)
{
	...
	if(curl_errno($curlL)==0) 
	{
		...
		while($xDay<$maxDay) 
		{
			...
			//Busco si existe el dia en cuestion en la base de datos, y actualizo los valores del pronostico
		/*	$query_exist=$wpdb->prepare("SELECT * FROM wp_an_clima_pronosticos WHERE cli_loc_id=%d AND cli_diaano=%d AND cli_ano=%d",$loc, $date_day+$xDay, $date_year);
			$exist=$wpdb->get_results($query_exist);
			if($wpdb->num_rows==0) //Si no existe lo ingreso
			{*/
			...
			/*}else //Si existe actualizo el valor
			{
				$wpdb->query($wpdb->prepare("UPDATE wp_an_clima_pronosticos SET cli_min=%d, cli_max=%d, cli_ico=%d, cli_estado=%s WHERE cli_loc_id=%d AND cli_diaano=%d AND cli_ano=%d", $Min, $Max, $Ico, $Estado, $loc, $date_day+$xDay, $date_year));

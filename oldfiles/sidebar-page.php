...
if(is_page()) 
{
	...
	$MaxAds=5;
	global $wpdb;
	$qb=$wpdb->get_results("SELECT a.ads_id AS id, b.anb_image AS img, b.anb_url AS link, s.anbs_height AS h, s.anbs_width AS w, a.ads_maxheight AS maxh, a.ads_maxwidth AS maxw FROM wp_an_ads_publicidad AS a LEFT JOIN wp_an_ads_banners AS b ON a.ads_anb_id=b.anb_id LEFT JOIN wp_an_ads_banners_size AS s ON b.anb_anbs_id=s.anbs_id WHERE a.ads_anp_id =3 ORDER BY a.ads_id",OBJECT_K);
	...
	if(($MaxAds-$cB)>0)
	{
		$queryA=$wpdb->prepare("SELECT a.ana_id AS id, a.ana_code AS code, s.anas_height AS h, s.anas_width AS w FROM wp_an_ads_adsense AS a LEFT JOIN wp_an_ads_adsense_size AS s ON a.ana_anas_id=s.anas_id WHERE a.ana_anp_id =3 ORDER BY a.ana_id LIMIT %d",intval($MaxAds-$cB));
